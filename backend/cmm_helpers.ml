...
module Generic_fns_tbl = struct
  ...
  module Precomputed = struct
    ...
    let is_curry (kind, arity, result) =
      if not (check_result result) then false else
      match kind with
      | Lambda.Tupled ->
      ...
      | Lambda.Curried { nlocal } ->
        ...
        if not in_bounds then false
        else if nlocal = 0 then true
        else if nlocal = 1 then true
        else if nlocal = l then true
        else if l <= 20 then true
        else false
  ...
  let gen () =
    ...
    in
    let send = List.init 20 (fun n ->
        [ (arity n, result, Lambda.alloc_local); (arity n, result, Lambda.alloc_heap) ]
      ) |> List.concat
    in
    let apply = List.init 404 (fun n ->
      [ (arity n, result, Lambda.alloc_local); (arity n, result, Lambda.alloc_heap) ]
    ) |> List.concat
    in
    let t = make () in
    add_uncached t Cmx_format.{
  ...
  let add t (Cmx_format.{ curry_fun; apply_fun; send_fun } as f) =
    ...
    | Some _ ->
      List.iter (fun f -> if not (Precomputed.is_curry f) then Hashtbl.replace t.curry f ()) curry_fun;
      List.iter (fun f -> if not (Precomputed.is_apply f) then Hashtbl.replace t.apply f ()) apply_fun;
      List.iter (fun f -> if not (Precomputed.is_send f) then Hashtbl.replace t.send f ()) send_fun
