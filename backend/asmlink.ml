...
let make_cached_startup_functions unix  ~ppf_dump ~sourcefile_for_dwarf =
  ...
  List.iter compile_phrase
    (Cmm_helpers.emit_preallocated_blocks []
      ...
      (*
  compile_phrase
    (Cmm_helpers.data_segment_table ([ startup_comp_unit ]));
  compile_phrase (Cmm_helpers.code_segment_table [ startup_comp_unit ]);
  Profile.record_call "frametable" (fun () ->
  compile_phrase (Cmm_helpers.frame_table [ startup_comp_unit ])); *)
  if !Clflags.output_complete_object then
    force_linking_of_startup ~ppf_dump;
  Emit.end_assembly ()
