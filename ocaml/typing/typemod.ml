...
let type_implementation sourcefile outputprefix modulename initial_env ast =
  ...
  Misc.try_finally (fun () ->
      ...
      if !Clflags.print_types then begin
        ...
        if Sys.file_exists sourceintf then begin
          ...
          Cms_format.save_cms (outputprefix ^ ".cms") modulename
            annots (Some sourcefile) None (Some shape);
          ...
          if not !Clflags.dont_write_files then begin
            ...
            Cms_format.save_cms (outputprefix ^ ".cms") modulename
              annots (Some sourcefile) (Some cmi) (Some shape);
            gen_annot outputprefix sourcefile annots
          end;
          { structure = str;
            coercion;
    ...
    ~exceptionally:(fun () ->
        ...
        Cms_format.save_cms (outputprefix ^ ".cms") modulename
          annots (Some sourcefile) None None;
        gen_annot outputprefix sourcefile annots
      )
...
let save_signature modname tsg outputprefix source_file initial_env cmi =
  ...
  Cms_format.save_cms (outputprefix ^ ".cmsi") modname
    (Cmt_format.Interface tsg) (Some source_file) (Some cmi) None
...
let package_units initial_env objfiles cmifile modulename =
  ...
  if Sys.file_exists mlifile then begin
    ...
    Cms_format.save_cms (prefix ^ ".cms") modulename
      (Cmt_format.Packed (sg, objfiles)) None None (Some shape);
    cc
  end else begin
    (* Determine imports *)
