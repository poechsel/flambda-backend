...
module Group = struct
  ...
  let pieces_of_code_for_cmx t =
    ...
    |> List.filter_map (fun code ->
           if Code.is_non_callable code then None else Some (Code.code_id code, code))
    |> Code_id.Map.of_list
