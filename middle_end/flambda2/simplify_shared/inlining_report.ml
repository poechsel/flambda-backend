...
module Context = struct
  ...
  let print ppf
    ...
    let print_args ppf args =
      ...
      in
      let print ppf args =
        if Inlining_arguments.is_oclassic args
        then Format.fprintf ppf "Oclassic"
        else if Inlining_arguments.is_o2 args
        then Format.fprintf ppf "O2"
        else if Inlining_arguments.is_o3 args
        then Format.fprintf ppf "O3"
        else Format.fprintf ppf "@[<h>%a@]" Table.print table
      in
      Format.fprintf ppf "@[<h>Inlining@ arguments: %a@]@," print args
    in
...
module Decision_with_context = struct
  ...
  let print_fundecl_details ppf
    ...
    | Never_inline_attribute ->
      Format.fprintf ppf "%a" Format.pp_print_text
        "@[<hov>The@ function@ has@ an@ attribute@ preventing@ it@ from@ being@ \
         inlinable.@]"
    | Function_body_too_large large_function_size ->
      Format.fprintf ppf
        "@[<hov>The@ function's@ body@ is@ too@ large,@ more@ specifically,@ it@ is@ \
         larger@ than@ the@ large@ function@ size:@ %a.@]"
        Code_size.print large_function_size
    | Stub -> Format.fprintf ppf "The@ function@ is@ a@ stub."
    | Attribute_inline ->
      Format.fprintf ppf
        "@[<hov>The@ function@ has@ an@ attribute@ forcing@ it@ to@ be@ inlined@ \
         when@ called.@]"
  ...
  let print_call_details fmt (decision : Call_site_inlining_decision_type.t) =
    ...
    | Definition_says_inline { was_inline_always = _ } ->
      Format.fprintf fmt
        "@[<hov>This@ function@ was@ decided@ to@ be@ always@ inlined@ at@ its@ \
         definition@ site (annotated@ by@ [@inlined always]@ or@ determined@ \
         to@ be@ small@ enough).@]"
